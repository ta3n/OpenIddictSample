...
) : IKeyRotationService
{
    ...
    )
    {
        ...
        var keyData = await cache.GetStringAsync(keyId);
    ...
    )
    {
        ...
        var keysJson = await cache.GetStringAsync(keysListId);
        ...
        var keyInfos = JsonSerializer.Deserialize<List<StoredKeyInfo>>(keysJson) ?? [];
    ...
    private async Task<SigningCredentials> GenerateAndStoreNewKeyAsync(
        string? tenantId = null
    )
    {
    ...
    private async Task AddToValidationKeysListAsync(
        StoredKeyInfo newKey,
        string? tenantId
    )
    {
        var keysListId = $"{KeysListPrefix}{tenantId ?? DefaultTenant}";
        ...
        var keyInfos = string.IsNullOrEmpty(keysJson)
            ? []
            : JsonSerializer.Deserialize<List<StoredKeyInfo>>(keysJson) ?? [];
