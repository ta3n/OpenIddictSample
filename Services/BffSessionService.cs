...
{
    Task<BffSession?> GetSessionAsync(string sessionId);
    Task CreateSessionAsync(string sessionId, BffSession session);
    Task UpdateSessionAsync(string sessionId, BffSession session);
    Task DeleteSessionAsync(string sessionId);
    Task<bool> RefreshSessionTokensAsync(string sessionId);
}
...
public class BffSessionService : IBffSessionService
{
    ...
    public async Task<BffSession?> GetSessionAsync(string sessionId)
    {
    ...
    public async Task CreateSessionAsync(string sessionId, BffSession session)
    {
        var key = SessionPrefix + sessionId;
    ...
    public async Task UpdateSessionAsync(string sessionId, BffSession session)
    {
    ...
    public async Task<bool> RefreshSessionTokensAsync(string sessionId)
    {
        ...
        try
        {
            ...
            var tokenEndpoint = _configuration["OpenIddict:TokenEndpoint"] ?? "https://localhost:5001/connect/token";

            var requestContent = new FormUrlEncodedContent(new[]
            {
                new KeyValuePair<string, string>("grant_type", "refresh_token"),
                new KeyValuePair<string, string>("refresh_token", session.RefreshToken),
                new KeyValuePair<string, string>("client_id", _configuration["OpenIddict:ClientId"] ?? "postman-client"),
                new KeyValuePair<string, string>("client_secret", _configuration["OpenIddict:ClientSecret"] ?? "postman-secret")
            });
